...
int verify_checkpoint(int8_t *area, int8_t *init_A_copy) {
    ...
    for (int offset = 0; offset < BITARRAY_SIZE; offset += 2) {
        int16_t current_word = *(int16_t *)(bitarray + offset) ;
        if (current_word == 0) {
            continue;
        }
        for (int k = 0; k < 16; k++) {
            if ((current_word >> k) & 1) {
                ...
                if (*(int64_t *)(init_A_copy + target_offset) != *(int64_t *)(areaS + target_offset)) {
                    fprintf(stderr,
                            "Checkpoint verify failed:\n"
                            "Offset 0x%x\n"
                            "Area S value: 0x%lx\n"
                            "Area A init Value: 0x%lx\n",
                            offset, *(int64_t *)(areaS + target_offset), *(int64_t *)(init_A_copy + target_offset));
                    return -1;
                }
            }
        }
...
void restore_area(int8_t *area) {
    ...
    for (int offset = 0; offset < BITARRAY_SIZE; offset += 8) {
        ...
        for (int i = 0; i < 8; i += 2) {
            current_word = *(int16_t *)(bitarray + offset + i);
            if (current_word == 0) {
                continue;
            }
            for (int k = 0; k < 16; k++) {
...
void restore_area_avx(int8_t *area) {
    int8_t *bitarray = area + 2 * SIZE;
    int8_t *src = area + SIZE;
    int8_t *dst = area;
    __m256i bitarray_vec;
    int16_t current_word;
    clock_t begin, end;
    double time_spent;
    begin = clock();
    ...
    for (int offset = 0; offset < BITARRAY_SIZE; offset += 32) {
        bitarray_vec = _mm256_loadu_si256((__m256i *)(bitarray + offset));
        if (_mm256_testz_si256(bitarray_vec, bitarray_vec)) {
            continue;
        }
        //loop for the 32 bytes
