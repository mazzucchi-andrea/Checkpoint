...
void audit_values(int8_t *area, int offset) {
  printf("Value written in A: %ld\n", *(int64_t *)(area + offset));
  printf("Value written in S: %lx\n", *(int64_t *)(area + 4096 + offset));
}
...
void test_checkpoint(int seed, int8_t *area, int64_t new_value,
  ...
  srand(seed);
  //offset = rand() % (4096 - 8 + 1);
  printf("Set new value (%ld)\n", new_value);
  for (int i = 0; i < numberOfWrites; i++) {
    offset = rand() % (4096 - 8 + 1);
    *(int64_t *)(area + offset) = new_value;
    //audit_values(area, offset);
  }
}
