...
void the_patch(unsigned long mem, unsigned long regs) {
    ...
    AUDIT
    // printf("memory access done by the application at instrumented instruction indexed by
...
// used_defined(...) is the real body of the user-defined instrumentation process, all the stuff you put here represents
// the actual execution path of an instrumented instruction given that you have the exact representation of the
// instruction to be instrumented, you can produce the block of ASM level instructions to be really used for
// istrumentation clearly any memory/register side effect is under the responsibility of the patch programme the
// instrumentation instructions whill be executed right after the original instruction to be instrumented
