...
int main(void) {
    ...
    file = fopen("mvm_test_results.csv", "a");
    if (file == NULL) {
        fprintf(stderr, "Error opening mvm_test_results.csv\n");
        return errno;
    }
    ...
    file = fopen("mvm_repeat_test_results.csv", "a");
    if (file == NULL) {
    ...
    for (int r = 2; r <= 10; r += 2) {
        ...
        mean_ci_95(ckpt_samples, &ckpt_mean, &ckpt_ci);
        mean_ci_95(restore_samples, &restore_mean, &restore_ci);
        fprintf(file, "0x%x,%d,%d,%d,%d,%d,%d,%.10f,%.10f,%.10f,%.10f\n",
